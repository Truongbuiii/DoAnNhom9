    ...
    <tbody>
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                // Tạo đường dẫn ảnh đúng thư mục ../img/
                $hinhAnhPath = "../img/" . htmlspecialchars($row['HinhAnh']);
                ...
                // Kiểm tra nếu có ảnh thì hiển thị, không thì báo "Không có ảnh"
                if (!empty($row['HinhAnh'])) {
                    echo "<img src='{$hinhAnhPath}' width='60' height='60' 
                               style='object-fit:cover; border-radius:8px;'>";
                } else {
                    echo "<span class='text-muted fst-italic'>Không có ảnh</span>";
                }
